use chumsky::Parser;
use parser::{markdown_parser, MarkdownNode, InlineMarkdownNode, LinkType};
        ...
        for (i, spanned) in parsed.body.iter().enumerate() {
            ...
            match &spanned.0 {
                MarkdownNode::Header { level, content } => {
                    println!("\n[{}] Header {} at bytes {}..{}", i, level, span.start, span.end);
                    ...
                    for (j, inline) in inlines.iter().enumerate() {
                        ...
                        match &inline.0 {
                            InlineMarkdownNode::PlainText(text) => {
                                println!("      [{}] PlainText '{}' at {}..{}", j, text, inline_span.start, inline_span.end);
                                println!("          Actual: '{}'", inline_text);
                                println!("          Match: {}", text == &inline_text);
                            }
                            InlineMarkdownNode::Link(link) => {
                                match link {
                                    LinkType::WikiLink { target, display_text, header } => {
                                        println!("      [{}] WikiLink '{}' at {}..{}", j, target, inline_span.start, inline_span.end);
                                        println!("          Actual: '{}'", inline_text);
                                        println!("          Match: {}", inline_text.contains(target));
                                    }
                                    LinkType::InlineLink { text, uri, header } => {
                                        println!("      [{}] InlineLink '{}' at {}..{}", j, uri, inline_span.start, inline_span.end);
                                        println!("          Actual: '{}'", inline_text);
                            ...
                            InlineMarkdownNode::Tag(tag) => {
                                println!("      [{}] Tag '{}' at {}..{}", j, tag, inline_span.start, inline_span.end);
                                println!("          Actual: '{}'", inline_text);
                            }
                            InlineMarkdownNode::Footnote(id) => {
                                println!("      [{}] Footnote '{}' at {}..{}", j, id, inline_span.start, inline_span.end);
                                println!("          Actual: '{}'", inline_text);
                            }
                            _ => {}
                        }
                ...
                MarkdownNode::FootnoteDefinition { id, content } => {
                    println!("\n[{}] FootnoteDef '{}' at bytes {}..{}", i, id, span.start, span.end);
                    println!("    Actual span text: '{}'", actual_text);
                }
                _ => {}
            }
