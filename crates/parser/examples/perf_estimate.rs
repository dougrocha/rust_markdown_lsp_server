...
fn benchmark_parse(name: &str, input: &str, iterations: usize) {
    ...
    println!("{:20} {:>6} lines, {:>8} bytes, {:>8} µs avg ({} iters, {}% success)",
             name, lines, bytes, avg_micros, iterations, (successful * 100) / iterations);
}

fn main() {
    println!("=== Parser Performance Estimates ===\n");
    println!("{:20} {:>6}        {:>8}       {:>8}", "Test", "Lines", "Bytes", "Time");
    println!("{}", "-".repeat(70));
